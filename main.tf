...
provider "proxmox" {
  pm_api_url = "https://pve.home.star4.io:8006/api2/json"
  pm_user = var.PM_USER
...
resource "proxmox_vm_qemu" "docker_server" {
  ...
  name = "docker-server" #count.index starts at 0, so + 1 means this VM will be named test-vm-1 in proxmox
...
resource "null_resource" "run-ansible" {
  ...
  provisioner "local-exec" {
    command = "cd docker-server && ./run.sh"
  }
